...
(`validate_terminal_grounding`)는 컷과 근거 체크포인트의 **겹침**만
기계로 요구한다 — 99–100s 체크포인트가 0–100s 컷을 승격시킬 수 있다.
범위 수렴으로 진입할 때는 에이전트가 **컷 span 전체가
terminal(verified/corrected)+support 체크포인트들로 덮이는지** 직접
확인하고, 덮이지 않은 부분은 먼저 검증한다.
