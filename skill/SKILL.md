...
| 사용자가 말하면 | 루프 | 진입 |
|---|---|---|
| 새 영상·이해·분석·"무슨 내용" | **데일리스**(1차 현상)→**큐브릭**(이해) | 절차 0 `va ingest --signals`→`va brief` |
| 하이라이트·쇼츠·"잘라줘" | 큐브릭→**쿨레쇼프**(편집)의 **에이젠슈테인 컷** | 이해 수렴 후 5의 클립 후보 평가 |
| "그 장면 어디였지"·기억 회상 | **스크립터**(회상) | `va search` — **재-ingest 금지** |
| 프리미어·편집 인계·마커 | 브릿지 | 5의 `va export` |
| 목록·위키·브라우저·외부 vault 갱신 | **랑글루아 서고**(지식 투영) | `va index`·`va wiki`·`va view`·`va bridge` |

촬영소 계보: 데일리스(현상)→스크립터(기록)→큐브릭(이해)→쿨레쇼프(편집)→
에이젠슈테인(몽타주)→랑글루아(서고).
